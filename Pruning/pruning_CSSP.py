...
def CSSP(method, M, criterion, value):
    ...
    if criterion == 'tol':
        match method:
            case "StrongRRQR":
                _, _, p, k, _ = sRRQR_tol(M_np, f=M_np.shape[1], tol=value)
                p = p[:k]
            case "ARP":
    ...
    M_hat = M_prune @ T                  # (n, m)
    relative_error = (np.linalg.norm(M_np - M_hat, ord="fro") / (np.linalg.norm(M_np, ord="fro") + 1e-12))
    p_tensor = torch.as_tensor(p, device=M.device)
    T_tensor = torch.from_numpy(T).to(device=M.device)

    return p_tensor, T_tensor, k, relative_error
...
def get_layerwise_retention(params, original_widths):
    """
    Return:
        {layer_idx: retained_ratio}
    """
    retention = {}
    ...
    for layer in params:
        ...
        if idx not in original_widths:
            continue

        current_width = layer["weight"].shape[0]
        original_width = original_widths[idx]

        retention[idx] = current_width / original_width
...
def layer_pruning(params, l, layer_type, global_idx, X, model, method, keep_rank, tol, device):
    ...
    match layer_type:
        case 'Linear' | 'LinearBNReLU':
            #if method == "StrongRRQR":
                #p, T, k, _ = CSSP(method, Z, 'tol', tol)    # T -> (k, m)
            #else:
                #p, T, k, _ = CSSP(method, Z, 'keep_rank', keep_rank)    # T -> (k, m)
            p, T, k, _ = CSSP(method, Z, 'keep_rank', keep_rank)    # T -> (k, m)
            # construct pruned parameters
        ...
        case 'Conv2d' | 'ConvBNReLU':                
            ...
            M = Z_reshaped.T
            #if method == "StrongRRQR":
                #p, T, k, _ = CSSP(method, M, 'tol', tol)    # T -> (k, m)
            #else:
                #p, T, k, _ = CSSP(method, M, 'keep_rank', keep_rank)    # T -> (k, m)
            p, T, k, _ = CSSP(method, M, 'keep_rank', keep_rank)    # T -> (k, m)
            # construct pruned parameters
            W_hat = W.index_select(0, p)    # 1st dimension of 'weight' -> output
            b_hat = b.index_select(0, p)
...
def iterative_pruning(model0, X, input_shape, rho, step_size, method, crit, test_loader, train_loader=None, S=None, tol=None, device=None, use_bn_recalibration=True, bn_batches=20):
    """
    Prune the model using CSSP-based method, for layers of type 'Linear' and 'Conv'
    Inputs:
        model0:         torch.nn.Module
    ...
    if crit == "flops":
        ...
        F = sum(p.numel() for p in model0.parameters())
        F0 = F
    ...
    for j in rho:
        ...
        while F > F0 * j:
            ...
            for i, layer in enumerate(params[:-1]):
                ...
                if crit == "flops":            
                    resource = compute_total_flops(model.model[begin: end+1], input_shape)
                elif crit == "params":
                    resource = sum(p.numel() for p in model.model[begin: end+1].parameters())
...
def evaluate_pruned_model(pruned_model, test_data, device=None):
    ...
    Output:
        ...
        total_params:  Total number of parameters in the pruned model
    """
    if device is None:
        try:
    ...
    return accuracy, wrong_samples, test_loss
...
def get_layer_activation_matrix(model, X, params, l):
    """
    l: index in params, not global layer index
    Return:
    ...
    if layer_type in ["Conv2d", "ConvBNReLU"]:
        ...
        A = Z.permute(0, 2, 3, 1).reshape(-1, Z.shape[1])
...
# Compare the relative Frobenius error of different CSSP methods for a specific layer under different keep_ratios
def compare_fixed_layer_relerr(model, X, l, methods, keep_ratios):
    """
    keep_ratios: list of float, e.g. [0.1, 0.3, 0.5, 0.7, 0.9]
    l : index in params, not global layer index
    """
    params = extract_params(model.model)
    ...
    for method in methods:
        ...
        for ratio in keep_ratios:
            k = int(m * ratio)
            _, _, _, relErr = CSSP(method, A, "keep_rank", k)
...
# Add best rank-k approximation error as a lower bound for comparison
def compute_svd_lower_bound(A, keep_ratios):
    A = A.detach().cpu().float()
    s = torch.linalg.svdvals(A)
    ...
    for ratio in keep_ratios:
        k = int(m * ratio)
        tail_norm = torch.sqrt(torch.sum(s[k:] ** 2))
        bounds.append((tail_norm / norm_A).item())
